...
\medskip\noindent
Let ($\Lambda$:VAR) be (a complete Noetherian local ring:TYPE) with residue field $k$,
...
\item[(B)] We let ($\Lambda$:VAR) be (a Noetherian ring:TYPE) and $\Lambda \to k$
...
Let ($k$:VAR) be (a finite type field:TYPE) over $S$, i.e., we are given a
...
Let ($\Lambda$:VAR) be (a Noetherian ring:TYPE) and let ($\Lambda \to k$:VAR) be (a finite
ring map:TYPE) where $k$ is a field. We define {\it $\mathcal{C}_\Lambda$} to be
the category with
\begin{enumerate}
...
Let ($f: B \to A$:VAR) be (a ring map:TYPE) in $\mathcal{C}_\Lambda$.  We say $f$
...
Let ($f: B \to A$:VAR) be (a surjective ring:TYPE) map in $\mathcal{C}_\Lambda$.
Then $f$ can be factored as a composition of small extensions.
\end{lemma}

\begin{proof}
Let ($I$:VAR) be (the kernel:TYPE) of $f$.  The maximal ideal $\mathfrak{m}_B$ is
...
:VAR)be (the image:TYPE) of our fixed finite ring map $\Lambda \to k$. Note
...
\begin{lemma}
\label{lemma-length}
Let ($A$:VAR) be (a local $\Lambda$-algebra with residue field:TYPE) $k$.
Let ($M$:VAR) be an $A$-module. Then
...
Let ($A \to B$:VAR) be (a ring map:TYPE) in $\mathcal{C}_\Lambda$.
...
Choose:VAR) $x \in A$ with $f(x) = y$. Since $f$ induces an isomorphism
...
Let ($R \to S$:VAR) be (a local homomorphism:TYPE) of local rings. The
...
Let ($p$:VAR) be (a prime number:TYPE) and let ($n \in \mathbf{N}$.
Let:VAR) ($\Lambda:VAR) = \mathbf{F}_p(t_1, t_2, \ldots, t_n)$ and let
$k = \mathbf{F}_p(x_1, \ldots, x_n)$ with map $\Lambda \to k$ given
by $t_i \mapsto x_i^p$. Let ($A = k[\epsilon] = k[x]/(x^2)$.
Then:VAR) $A$ is an object of $\mathcal{C}_\Lambda$. Suppose that
...
Let ($f_1 : A_1 \to A$ and:VAR) $f_2 : A_2 \to A$ be ring maps in
...
Let ($P'(T) \in k'[T]$:VAR) be (the minimal polynomial:TYPE) of $\overline{x}$ over $k'$.
...
Let ($f: B \to A$:VAR) be (a ring map:TYPE) in $\mathcal{C}_\Lambda$.
The following are equivalent
\begin{enumerate}
...
\begin{proof}
Assume (3). Let ($C \to B$:VAR) be (a ring map:TYPE) in $\mathcal{C}_\Lambda$ such
...
:VAR)be (a morphism:TYPE) of $\mathcal{C}_\Lambda$ such that
...
\medskip\noindent
Assume (2)(b). Let ($g : C \to B$:VAR) be (a ring map:TYPE) in $\mathcal{C}_\Lambda$
...
is surjective. Let ($K$:VAR) be (the kernel:TYPE). Note that
...
$C' \subset B$ be (the image:TYPE) of $C \to B$. Then $C' \not = B$ but
...
Let ($\Lambda = k[[x]]$:VAR) be (the power series:TYPE) ring in $1$ variable over $k$.
...
Let ($\Lambda$:VAR) be (a Noetherian ring:TYPE) and let ($\Lambda \to k$:VAR) be (a finite
ring map:TYPE) where $k$ is a field. We define {\it $\widehat{\mathcal{C}}_\Lambda$}
...
Let ($f: R \to S$:VAR) be (a ring map:TYPE) in $\widehat{\mathcal{C}}_\Lambda$.
The following are equivalent
\begin{enumerate}
\item $f$ is surjective,
\item the map
...
Now let ($K_n$:VAR) be (the kernel:TYPE) of $R_n \to S_n$.  Then the sequences
$$
...
\begin{proof}
Let ($R \to S_1$:VAR) and $R \to S_2$ be morphisms of
...
Let ($S$:VAR) be (an object:TYPE) of $\widehat{\mathcal{C}}_\Lambda$.
Then $\dim_k \text{Der}_\Lambda(S, k) < \infty$.
\end{lemma}

\begin{proof}
Let ($x_1, \ldots, x_n \in \mathfrak m_S$ map:VAR) to a $k$-basis
...
Let ($f : R \to S$:VAR) be (a morphism:TYPE) of $\widehat{\mathcal{C}}_\Lambda$.
...
Let $\ldots \to A_3 \to A_2 \to A_1$ be (a sequence:TYPE) of surjective
...
(whose images:TYPE) in $k$ generate $k$ over $\Lambda$. Then we get a ring map
...
:VAR)be the kernel of $P \to k$. Let $R = P^\wedge$ be (the $\mathfrak m$-adic:TYPE)
...
we see that $(R/J =:TYPE) \lim (R/J_n:TYPE) = \lim A_n = S$
...
:VAR)$R = R' \oplus I$ for some ideal $I$ of $R$. Let ($x_1, \ldots, x_r \in I$
map:VAR) to a basis of $I/\mathfrak m_R I$. Set $S = R'[[X_1, \ldots, X_r]]$
...
In developing the theory we work with categories {\it cofibered} in groupoids.
 We assume as known the definition and basic properties of (categories:TYPE)
...
Let ($\mathcal{C}$:VAR) be (a category:TYPE).  A {\it category cofibered in groupoids over
...
Let ($\mathcal{C}$:VAR) be (a category:TYPE).
\begin{enumerate}
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
$\mathcal{C}$, let ($f: U \to V$:VAR) be (a morphism:TYPE) in $\mathcal{C}$, and
let ($x \in \Ob(\mathcal{F}(U))$.
A {\it pushforward}:VAR) of $x$ along $f$ is a morphism
...
\item Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
...
Let ($F : \mathcal{C} \to \textit{Groupoids}$:VAR) be (a functor:TYPE). There
...
Let ($\mathcal{F}$:VAR) be cofibered in groupoids over $\mathcal{C}$.
...
Let ($F: \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE). We can think of a
...
Let ($U$:VAR) be (an object:TYPE) of $\mathcal{C}$.  We write $\underline{U}$ for the
functor
...
Restricting the base category: Let ($p : \mathcal{F} \to \mathcal{C}$:VAR) be a
category cofibered in groupoids, and let ($\mathcal{C}'$:VAR) be (a full
subcategory:TYPE) of $\mathcal{C}$. The restriction $\mathcal{F}|_{\mathcal{C}'}$
...
Let ($F : \mathcal{C}_\Lambda \to \textit{Sets}$:VAR) be (a functor:TYPE).
...
Let ($x_0 \in \Ob(\mathcal{F}(k))$. Then every object:VAR) of
...
Let ($p : \mathcal{F} \to \mathcal{C}_\Lambda$:VAR) be (a category:TYPE) cofibered in
groupoids, and let ($x \in \Ob(\mathcal{F}(k))$.  We denote:VAR) by
...
\item Let ($\xi = (R, \xi_n, f_n)$ and:VAR) $\eta = (S, \eta_n, g_n)$ be
...
\begin{proof}
Let ($R \to S$:VAR) be (a ring map:TYPE) in $\widehat{\mathcal{C}}_\Lambda$.
Let ($(R, \xi_n, f_n)$:VAR) be (an object:TYPE) of $\widehat{\mathcal{F}}$.
...
can be defined as follows.  For each $n$, let ($m(n)$:VAR) be (the least $m$:TYPE)
that $I_m \subset \mathfrak m_R^n$.  Given an object
$(\xi_n, f_n)$ of $\widehat{\mathcal{F}}_\mathcal{I}(R)$, let
$\eta_n$ be (the pushforward:TYPE) of $\xi_{m(n)}$ along
$(R/I_{m(n):TYPE)} \to R/\mathfrak m_R^n$. Let ($g_n : \eta_{n + 1} \to \eta_n$
:VAR)be (the unique morphism:TYPE) of $\mathcal{F}$ lying over
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibred in groupoids over
...
\mathcal{F}.
$$
Namely, let ($A \in \Ob(\mathcal{C}_\Lambda)$ and let:VAR)
$(A, \xi_n, f_n)$ be (an object:TYPE) of
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$:VAR) be (a morphism:TYPE) of (categories:TYPE)
...
be (an object:TYPE) of the fiber category $\mathcal{G}(R)$.
...
:VAR)be a functor that commutes with limits.
...
Let ($R$:VAR) be (an object:TYPE) of $\widehat{\mathcal{C}}_\Lambda$.  It defines a functor
...
:VAR)$\eta = (S, \eta_n, g_n)$ be formal objects of $\mathcal{F}$.
Let $a = (a_n) : \xi \to \eta$ be (a morphism:TYPE) of formal objects, i.e.,
a morphism of $\widehat{\mathcal{F}}$. Let
$f = \widehat{p}(a) = a_0 : R \to S$ be (the projection:TYPE) of $a$ in
...
To see this let ($\alpha : S \to A$:VAR) be (an object:TYPE) of
...
\to \varphi(x)$ be (a morphism:TYPE) lying over $B \to A$.  Then there
...
\begin{proof}
Let $B \to A$ be (a surjective ring map:TYPE) in $\mathcal{C}_\Lambda$.
Let ($y \in \Ob(\mathcal{G}(B))$,:VAR) $x \in \Ob(\mathcal{F}(A))$,
and $y \to \varphi(x)$ be (a morphism:TYPE) lying over $B \to A$. By
...
Let ($x \in \Ob(\mathcal{F}(A))$ and let:VAR)
$\overline{y} \in \overline{\mathcal{F}}(B)$
be (the isomorphism class:TYPE) of $y \in \Ob(\mathcal{F}(B))$ such that
...
Let ($R \to S$:VAR) be (a ring map:TYPE) in $\widehat{\mathcal{C}}_\Lambda$. Then
...
Conversely, assume
$\underline{S}|_{\mathcal{C}_\Lambda} \to \underline{R}|_{\mathcal{C}_\Lambda}$
is smooth. Let ($x_1, \ldots, x_n \in S$:VAR) be elements whose images
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$ and:VAR) $\psi : \mathcal{G}
...
\item If $\mathcal{G}' \to \mathcal{G}$ is a morphism of (categories:TYPE)
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$:VAR) be (a smooth morphism:TYPE) of
...
\begin{proof}
Let ($y$:VAR) be (an object:TYPE) of $\mathcal{G}$ lying over
$A \in \Ob(\mathcal{C}_\Lambda)$. Let ($y \to y_0$:VAR) be (a pushforward:TYPE)
...
\medskip\noindent
Let ($\eta = (R, \eta_n, g_n)$:VAR) be (an object:TYPE) of $\widehat{\mathcal{G}}$.  We
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids.  Let ($\xi$:VAR) be (a formal
object:TYPE) of $\mathcal{F}$ lying over $R \in \Ob(\widehat{\mathcal{C}}_\Lambda)$.
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over $\mathcal
C_\Lambda$, and let ($\xi$:VAR) be (a formal object:TYPE) of $\mathcal{F}$.  It follows
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE).
Let ($\xi$:VAR) be (a versal formal object:TYPE) of $\mathcal{F}$.
...
equivalent
\begin{enumerate}
\item:VAR) $\underline{R}|_{\mathcal{C}_\Lambda}$ is smooth,
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE).
Let ($\xi$:VAR) be (a versal formal object:TYPE) of $\mathcal{F}$ lying over
...
(For example, let ($P$:VAR) be (a polynomial algebra:TYPE).)
...
\medskip\noindent
Let ($R$:VAR) be (the $\mathfrak m_P$-adic:TYPE) completion of $P$.
...
that such a fibre product may not always be (an object:TYPE) of
...
diagram. To see this we let ($y \to y_V$:VAR) be (a morphism:TYPE) lying over
...
Then (we:TYPE) have commutative diagrams
$$
\vcenter{
\xymatrix{
...
the pushforwards:VAR) $f_{1, *}(x_1)$ and $f_{2, *}(x_2)$ are isomorphic.
...
Let:VAR) ($\mathcal{F}_{x_0}$:VAR) be (the category:TYPE) cofibred in groupoids over
...
\noindent
Let ($R$:VAR) be (a ring:TYPE). We write $\text{Mod}_R$ for the category of
...
resp.\:VAR) $L: \text{Mod}_R \to \text{Mod}_R$
...
resp.\:VAR) $L: \text{Mod}_R \to \textit{Sets}$ be a
...
Let ($M$:VAR) be (a finitely:TYPE) generated $R$-module. We define $\widetilde{L}(M)$ to be
...
Let ($L_1, L_2: \text{Mod}^{fg}_R \to \textit{Sets}$:VAR) be
functors that take $0$ to a one element set and preserve finite products.
Let ($t : L_1 \to L_2$:VAR) be (a morphism:TYPE) of functors. Then $t$ induces a morphism
...
Let ($K$:VAR) be (a field:TYPE). Let ($L: \text{Mod}^{fg}_K \to
\text{Mod}_K$:VAR) be (a $K$-linear functor:TYPE).  Then $L$ is isomorphic to the
...
\noindent
For a ring $R$ and an $R$-module $M$, let ($R[M]$:VAR) be (the $R$-algebra:TYPE) whose
...
\medskip\noindent
Now let ($S$:VAR) be (a ring:TYPE) and assume $R$ is an $S$-algebra.
...
Let ($R$:VAR) be (an $S$-algebra:TYPE). Then the functor
...
Let ($R$:VAR) be (an $S$-algebra:TYPE), and let ($\mathcal{C}$:VAR) be (a strictly full:TYPE)
subcategory of $S\text{-Alg}/R$ containing $R[M]$ for all
$M \in \Ob(\text{Mod}^{fg}_R)$.
Let ($F: \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE). Suppose that
...
by the assumption on $\mathcal{C}$. Let ($L$:VAR) be (the composition:TYPE)
...
Let ($\mathcal{C}$:VAR) be (a category:TYPE) as in
Lemma \ref{lemma-tangent-space-functor}.
Let ($F : \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE) such that
...
the hypotheses of
Lemma \ref{lemma-tangent-space-functor}.
Let ($t : F \to G$:VAR) be (a morphism:TYPE) of functors. For any
...
Let ($F: \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE) satisfying the
hypotheses of
Lemma \ref{lemma-tangent-space-functor}.
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE) such that
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$:VAR) be (a morphism:TYPE) predeformation
categories. The
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$:VAR) be (a morphism:TYPE) of predeformation
...
categories cofibered in groupoids as follows. Let ($\mathcal{F}$:VAR) be (a category:TYPE)
...
$$
Namely, for $D \in \text{Der}_\Lambda(k, k)$ let ($f_D:VAR) : k \to k[\epsilon]$
...
\medskip\noindent
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE) over $\mathcal{C}_\Lambda$.
Let ($x_0 \in \Ob(\mathcal{F}(k))$. By the above there:VAR) is a canonical
map
$$
\gamma_V :
\text{Der}_\Lambda(k, V)
...
Lemma \ref{lemma-morphism-linear-functors}.
Let ($D : k \to V$:VAR) be (a $\Lambda$-derivation:TYPE).
...
\begin{proof}
Let ($\xi$:VAR) be (a versal formal object:TYPE) of $\mathcal{F}$. Let
$$
\xymatrix{
           & x_2 \ar[d] \\
x_1 \ar[r] & x
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE) satisfying (S1) and
(S2). Let ($\xi$:VAR) be (a formal object:TYPE) of $\mathcal{F}$ corresponding to
...
$$
commutes. Let ($b : R \to B$:VAR) be (the map:TYPE) guaranteed by (2). Denote
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibred in groupoids over
$\mathcal{C}_\Lambda$ which has (S1). Let ($B \to A$:VAR) be (a surjection:TYPE)
in $\mathcal{C}_\Lambda$ with kernel $I$ annihilated by $\mathfrak m_B$.
Let ($x \in \mathcal{F}(A)$. The set:VAR) of ideals
$$
...
\medskip\noindent
Step 1. Let ($J_1 = \mathfrak m_S$. Let:VAR) ($\xi_1:VAR)$ be (the unique:TYPE)
...
T\mathcal{F}
$$
is surjective. Let ($f_1 : \xi_2 \to \xi_1$:VAR) be (the unique morphism:TYPE).
...
Let ($f_n : \xi_{n + 1} \to \xi_n$:VAR) be any morphism of $\mathcal{F}$
...
(b) of (the construction:TYPE) of $J_{n + 1}$ in the induction step above.
...
Let ($F : \mathcal{C}_\Lambda \to \textit{Sets}$:VAR) be (a predeformation:TYPE) functor
...
and let ($y' \to x'$:VAR) be over $B' \to f(B')$ be (the pushforward:TYPE) of $y'$.
...
\begin{proof}
Proof of (1). Say ($y \to x$ lies over:VAR) $f : B \to A$. Let ($y' \to y$
lying:VAR) over $B' \subset B$ be (a minimal object:TYPE) of $\mathcal{S}_y$. Let
$$
\vcenter{
\xymatrix{
y' \ar[d] \ar[r] & x' \ar[d] \\
...
Let ($x' \to x$:VAR) be (a minimal object:TYPE) of $\mathcal{S}_x$ lying over $A' \subset A$.
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibred in groupoids over
$\mathcal{C}_\Lambda$ which has (S1). Let ($\xi$:VAR) be (a versal formal object:TYPE)
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibred in groupoids over
$\mathcal{C}_\Lambda$ which has (S1). Let ($\xi$:VAR) be (a versal formal object:TYPE)
of $\mathcal{F}$ lying over $R$. Let ($\xi' \to \xi$:VAR) be (a morphism:TYPE)
...
Let ($x_1, \ldots, x_n \in I$:VAR) be elements which form a basis for
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE) which
...
Let ($\xi' \to \xi$ over $:VAR)(R:VAR)' \subset R$ be any of the morphisms constructed in
Lemma \ref{lemma-smallest-where-descends-versal}.
By
...
Let $\xi$ be (a formal object:TYPE) of the predeformation category
$\mathcal{F}$ and let
$\underline{\xi} : \underline{R}|_{\mathcal{C}_\Lambda} \to \mathcal{F}$
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE).
Let ($\xi$:VAR) be (a versal formal object:TYPE) of $\mathcal{F}$ such that
...
structure morphism. Say:VAR) ($\widehat{p}(\xi:VAR)) = R$, i.e., $\xi$ lies over
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE) satisfying
(S2) which has a versal formal object. Then its minimal versal
formal object satisfies (\ref{equation-bijective-orbits}).
\end{lemma}

\begin{proof}
Let ($\xi$:VAR) be (a minimal versal formal object:TYPE) for $\mathcal{F}$, see
Lemma \ref{lemma-minimal-versal}.
...
\medskip\noindent
Let ($\xi \to y$:VAR) be (a morphism:TYPE) lying over $R \to R/\mathfrak m_R^2$.
Let ($y \to x$:VAR) be (a morphism:TYPE) lying over
$R/\mathfrak m_R^2 \to R'/\mathfrak m_R^2$.
Let ($y \to x_\epsilon$:VAR) be (a morphism:TYPE) lying over
$R/\mathfrak m_R^2 \to k[\epsilon]$. Let ($x_0$:VAR) be (the unique:TYPE) (up to unique
...
Let ($\mathcal{F}$:VAR) be (a predeformation category:TYPE). Recall that
...
\medskip\noindent
Assume (3). Let ($q_1 : A_1 \to A$ and:VAR) $q_2 : A_2 \to A$ be given with
...
Categories, Remark \ref{categories-remark-other-description-2-fibre-product}.
Hence let ($y \in \mathcal{F}(A_1 \times_A A_2)$ correspond:VAR) to
$(x_1, x_2, x, a_1 : x_1 \to x, a_2 : x_2 \to x)$.
Let ($z$:VAR) be (an object:TYPE) of $\mathcal{F}$ lying over $C$. Then
\begin{align*}
\Mor_\mathcal{F}(z, y) & =
...
\item Let ($f_1: A_1 \to A$ and:VAR) $f_2: A_2 \to A$ be ring maps in
...
\medskip\noindent
Assume (2) holds. Let ($x' \to x$:VAR) be (a morphism:TYPE) in $\mathcal{F}$ lying
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
$\mathcal{C}_\Lambda$. Let ($f: A' \to A$:VAR) be a map in $\mathcal{C}_\Lambda$.
Let ($x \in \mathcal{F}(A)$. The category:VAR) $\textit{Lift}(x, f)$ of lifts of $x$
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibred in groupoids over
$\mathcal{C}_\Lambda$. Let ($x_0 \in \Ob(\mathcal{F}(k))$.
Let:VAR) ($V$:VAR) be (a finite dimensional vector space:TYPE).
...
$$
be (a composition:TYPE) of fibre squares in $\mathcal{C}_\Lambda$ with
both $A'_1 \to A_1$ and $A_1 \to A$ surjective. Let ($x_1 \to x$:VAR) be (a morphism:TYPE)
...
Let ($\mathcal{F}$:VAR) be (a deformation category:TYPE).
Let ($A' \to A$:VAR) be (a surjective ring:TYPE) map in
$\mathcal{C}_\Lambda$ whose kernel $I$ is annihilated
by $\mathfrak m_{A'}$. Let ($x \in \Ob(\mathcal{F}(A))$.
If:VAR) $\text{Lift}(x, A')$ is nonempty,
...
Let ($\varphi : \mathcal{F} \to \mathcal{G}$:VAR) be (a morphism:TYPE) of deformation
categories. Let ($A' \to A$:VAR) be (a surjective ring map:TYPE) whose kernel $I$
...
functors. Let ($\varphi : F \to G$:VAR) be (a smooth morphism:TYPE) which induces
...
statement holds for rings of length less than $n$ and let ($A'$:VAR) be (a ring:TYPE) of
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over $\mathcal
C_\Lambda$. Let ($x' \to x$:VAR) be (a morphism:TYPE) in $\mathcal{F}$ lying over
$A' \to A$. The kernel
$$
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over $\mathcal
C_\Lambda$. Let ($x_0 \in \Ob(\mathcal{F}(k))$. Assume a choice:VAR) of
...
Let ($p : \mathcal{F} \to \mathcal{C}$:VAR) be (a category:TYPE) cofibered in groupoids
over an arbitrary base category $\mathcal{C}$. Assume a choice of pushforwards
has been made. Let ($x \in \Ob(\mathcal{F})$ and let:VAR) ($U = p(x)$.
Let:VAR) $U/\mathcal{C}$ denote the category of objects under $U$. The
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
$\mathcal{C}_\Lambda$ satisfying (RS). Let
$x \in \Ob(\mathcal{F}(A))$. Let ($x_0$:VAR) be (a pushforward:TYPE) of $x$ to
$\mathcal{F}(k)$.
\begin{enumerate}
...
$\mathcal{F}$ be (a category:TYPE) cofibered in groupoids over $\mathcal{C}_\Lambda$.
Let ($x' \to x$:VAR) be (a morphism:TYPE) lying over a ring map $A' \to A$.
...
Then:VAR) $\text{Inf}_{x_0}(\mathcal{F})$ is equal as a set to
...
Let ($x_0 \in \Ob(\mathcal{F}(k))$. Then:VAR) $\varphi$ induces a $k$-linear
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
$\mathcal{C}_\Lambda$ satisfying (RS). Let ($x' \to x$:VAR) be a
morphism lying over a surjective ring map $A' \to A$ with kernel $I$
annihilated by $\mathfrak m_{A'}$. Let ($x_0$:VAR) be (a pushforward:TYPE) of $x$ to
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over
$\mathcal{C}_\Lambda$ satisfying (RS). Let ($x' \to x$:VAR) be (a morphism:TYPE)
in $\mathcal{F}$ lying over a surjective ring map. Let ($x_0$:VAR) be (a pushforward:TYPE)
...
$\text{Inf}_{x_0}(\mathcal{F}) = 0$.  Let ($A$:VAR) be (an object:TYPE) of
...
Let ($f : \mathcal{H} \to \mathcal{F}$ and:VAR) $g : \mathcal{G} \to \mathcal{F}$
be $1$-morphisms of deformation (categories:TYPE). Then
\begin{enumerate}
...
\medskip\noindent
Part (2). Let ($w_0 \in \Ob(\mathcal{W}(k))$ and let:VAR) ($x_0, y_0, z_0$:VAR) be
...
Let ($\mathcal{H}_1 \to \mathcal{G}$,:VAR) $\mathcal{H}_2 \to \mathcal{G}$, and
...
Let ($A' \to A$:VAR) be (a small extension:TYPE) in $\mathcal{C}_\Lambda$.
...
Let ($f : \mathcal{F} \to \mathcal{G}$:VAR) be a map of deformation
categories. Let ($x_0 \in \Ob(\mathcal{F}(k))$ with image
:VAR)$y_0 \in \Ob(\mathcal{G}(k))$. If
\begin{enumerate}
...
\begin{proof}
Let ($A' \to A$:VAR) be (a small extension:TYPE). Let ($x \in \mathcal{F}(A)$.
Let:VAR) ($y' \to f(x)$:VAR) be (a morphism:TYPE) in $\mathcal{G}$ over $A' \to A$.
...
Let ($\mathcal{F} \to \mathcal{G} \to \mathcal{H}$:VAR) be maps of (categories:TYPE)
...
\begin{proof}
Let ($A' \to A$:VAR) be (a small extension:TYPE) in $\mathcal{C}_\Lambda$
and let ($x \in \mathcal{F}(A)$ with image:VAR) $y \in \mathcal{G}(A)$.
...
Let ($\mathcal{C}$:VAR) be (a category:TYPE). The
...
Let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on a category $\mathcal{C}$.
...
Let ($\mathcal{C}$:VAR) be (a category:TYPE). A groupoid in functors on $\mathcal{C}$ is
...
We will say ``{\it let ($(\underline{U}, \underline{R}, s, t, c)$:VAR) be a
...
Let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on a category $\mathcal{C}$.
Let ($\mathcal{C}'$:VAR) be (a subcategory:TYPE) of $\mathcal{C}$. The
...
\noindent
Let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on $\mathcal{C}_\Lambda$.
...
Remark \ref{remark-restrict-completion}.
On the other hand, let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on
...
Let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on $\mathcal{C}_\Lambda$. We
...
$[U/R]$ need not be (a smooth category:TYPE) cofibred in groupoids as defined in
...
:VAR)be a prorepresentable groupoid in functors on $\mathcal{C}_\Lambda$.
...
\end{enumerate}
If (4) holds and $f, a, y$ as in (2) are given, let ($x = t(a)$:VAR) so that
...
:VAR)as in (4) be given. Then (2) produces $a' \in R(B)$, and $x' = t(a')$ and
...
Let ($(U, R, s, t, c)$:VAR) be (a smooth groupoid:TYPE) in functors on $\mathcal{C}_\Lambda$.
...
Let ($p_1, p_2$:VAR) be (the projections:TYPE) of $A_1 \times_A A_2$.  We claim
$$
\xymatrix{
...
Let ($\mathcal{F}$:VAR) be (a category:TYPE) cofibered in groupoids over a category
$\mathcal{C}$. Let ($(U, R, s, t, c)$:VAR) be (a groupoid:TYPE) in functors on
$\mathcal{C}$. A
...
Let ($\mathcal{F}$:VAR) be category cofibered in groupoids over a category
$\mathcal{C}$. Let ($U : \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE).
Let ($f : U \to \mathcal{F}$:VAR) be (a morphism:TYPE) of categories cofibered in groupoids
...
$\mathcal{C}$. Let ($U : \mathcal{C} \to \textit{Sets}$:VAR) be (a functor:TYPE).
Let ($f : U \to \mathcal{F}$:VAR) be (a morphism:TYPE) of categories cofibered in groupoids
over $\mathcal{C}$. Let ($(U, R, s, t, c)$:VAR) be (the groupoid:TYPE) in functors on
...
:VAR)be a functor. Let ($f : U \to \mathcal{F}$:VAR) be (a smooth morphism:TYPE) of
...
Let ($\mathcal{F}$:VAR) be (a deformation category:TYPE).
Let ($U : \mathcal{C}_\Lambda \to \textit{Sets}$:VAR) be (a deformation:TYPE) functor.
Let ($f: U \to \mathcal{F}$:VAR) be (a morphism:TYPE) of categories cofibered in groupoids.
...
Let ($\mathcal{F}$:VAR) be (a deformation category:TYPE).
Let ($U : \mathcal{C}_\Lambda \to \textit{Sets}$:VAR) be (a prorepresentable functor:TYPE).
Let ($f : U \to \mathcal{F}$:VAR) be (a morphism:TYPE) of categories cofibered in groupoids.
Let ($(U, R, s, t, c)$:VAR) be (the groupoid:TYPE) in functors on $\mathcal{C}_\Lambda$
...
Let ($(U, R, s, t, c)$:VAR) be (the groupoid:TYPE) in functors constructed in
Lemma \ref{lemma-presentation-construction}
...
Let ($(U, R, s, t, c)$:VAR) be (a smooth prorepresentable groupoid:TYPE) in functors
...
Let ($(U, R, s, t, c)$:VAR) be (a smooth prorepresentable groupoid:TYPE) in
...
Let ($U: \mathcal{C}_\Lambda \to \textit{Sets}$:VAR) be a
prorepresentable functor. Let ($\varphi : U \to U$:VAR) be (a morphism:TYPE) such
...
Let ($(U, R, s, t, c)$:VAR) be (a minimal smooth prorepresentable groupoid:TYPE) in
...
Let ($(U, R, s, t, c)$ and:VAR) $(U', R', s', t', c')$ be minimal smooth
...
\begin{proof}
Let ($\psi : [U'/R'] \to [U/R]$:VAR) be (a quasi-inverse:TYPE) to $\varphi$.
...
Let ($\mathcal{F}$:VAR) be (a deformation:TYPE) category such that
...
Let:VAR) ($f = \underline{\xi} : U \to \mathcal{F}$:VAR) be (the associated
morphism:TYPE). Let ($(U, R, s, t, c)$:VAR) be (the groupoid:TYPE) in functors on
$\mathcal{C}_\Lambda$ constructed from $f : U \to \mathcal{F}$ in
Lemma \ref{lemma-presentation-construction}.
...
Let ($\mathcal{F}$:VAR) be category cofibered in groupoids over
...
Let ($\mathcal{F}$:VAR) be (a deformation:TYPE) category over $\mathcal{C}_\Lambda$
with $\dim_k T\mathcal{F} < \infty$ and
$\dim_k \text{Inf}(\mathcal{F}) < \infty$. Let ($\xi$:VAR) be (a versal formal
object:TYPE) lying over $R$. Let ($\eta$:VAR) be (a formal object:TYPE) lying over $S$.
Then any two maps
$$
f, g : R \to S
$$
...
field $k$ by a finite extension. Let ($\Lambda$:VAR) be (a Noetherian ring:TYPE) and
let ($\Lambda \to k$:VAR) be (a finite ring map:TYPE) where $k$ is a field. Throughout
...
Let ($\Lambda$:VAR) be (a Noetherian ring:TYPE) and let ($\Lambda \to k \to l$:VAR) be (a finite
ring maps:TYPE) where $k$ and $l$ are fields. Thus $l/k$ is a finite
...
\begin{proof}
Let ($x_0 \in \Ob(\mathcal{F}(k))$ and denote:VAR) $x_{l, 0}$ the corresponding
...
$\mathcal{F}_{l/k} \to \mathcal{G}_{l/k}$ is a smooth morphism of (categories:TYPE)
...
With notation and assumptions as in Situation \ref{situation-change-of-fields}.
Let $\xi$ be (a versal formal object:TYPE) for $\mathcal{F}$ lying over
